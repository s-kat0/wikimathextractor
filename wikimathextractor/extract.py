...
class Template(list):
    ...
    def subst(self, params, extractor, depth=0):
        ...
        logging.debug(
            "subst tpl (%d, %d) %s", len(extractor.frame), depth, self
        )
...
class Extractor:
    ...
    def clean_text(
        ...
        :param mark_headers: True to distinguish headers from paragraphs
          e.g. "## Section 1"
        """
        self.magicWords["namespace"] = self.title[
            : max(0, self.title.find(":"))
        ]
    ...
    def expandTemplates(self, wikitext):
        ...
        for s, e in findMatchingBraces(wikitext, 2):
            res += wikitext[cur:s] + self.expandTemplate(
                wikitext[s + 2 : e - 2]
            )
            cur = e
        # leftover
    ...
    def templateParams(self, parameters):
        ...
        logging.debug(
            "   templateParams> %s", "|".join(templateParams.values())
        )
        return templateParams
...
parserFunctions = {
    ...
    "padleft": lambda char, width, string: string.ljust(
        char, int(pad)
    ),  # CHECK_ME
}
